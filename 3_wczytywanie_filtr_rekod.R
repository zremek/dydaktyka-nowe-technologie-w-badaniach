...
### wczytajmy plik pobrany przez API YouTube za pomocą https://ytdt.digitalmethods.net/mod_videos_list.php 

videolist <- read_csv("videolist_search1125_2026_04_10-10_22_15.csv")
# możemy wczytać plik csv podając tylko jego nazwę za pomocą funkcji
# read_csv() z pakietu tidyverse 

# UWAGA: aby wczytać pliki z danymi tak, jak w podanych w tym skrypcie przykładach,
# należy utworzyć projekt w RStudio, zaś pliki umieścić w folderze projektu.
# alternatywą jest podawanie całej ścieżki dostępu do pliku, 
# np. "C:/Users/Janusz/jakiś_folder/mój_plik.csv"


summary(videolist$viewCount) # tutaj przykładowe statystyki opisowe na wczytanym pliku 

# możemy wczytać każdy plik csv, w kolejnym przykładzie to plik z Google Trends
# lecz czasami trzeba zastosować dodatkowe opcje wczytania do R
# z powodu specyfiki wczytywanego pliku 

gtrends <- read_csv(file = "multiTimeline(2).csv", 
           skip = 2) # omijamy dwa pierwsze wiersze, ponieważ są puste 

summary(gtrends)

### poprawmy nazwy zmiennych

summary(gtrends$`Pierre Bourdieu: (Cały świat)`) 
# nazwa zawiera m.in spacje, nie pracuje się z tym wygodnie w R
...
# tutaj zmieniamy liczby z etykietami na "factor" czyli typowy dla R format 
